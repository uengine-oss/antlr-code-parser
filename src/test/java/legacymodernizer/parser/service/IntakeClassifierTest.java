...
import legacymodernizer.parser.service.IntakeClassifier.Kind;

/**
 * IntakeClassifier 단위 테스트 — 내용 기반 표 정의(DDL) vs 소스 분류(헌법 III).
 *
 * <p>핵심: 프로시저/함수가 든 .sql 은 절대 DDL 로 분류되지 않는다(analyzer sqlglot 크래시 차단).
 */
class IntakeClassifierTest {

    private final IntakeClassifier classifier = new IntakeClassifier();
